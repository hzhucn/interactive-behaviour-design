...
from utils import EnvState, get_noop_action, save_video, make_small_change, \
    find_latest_checkpoint, unwrap_to
from wrappers.util_wrappers import ResetMode, DummyRender


class RolloutWorker:
    def __init__(self, make_policy_fn_pickle, log_dir, env_state_queue, rollout_queue, no_render_demonstrations):
        ...
        while True:
            ...
            if no_render_demonstrations and 'DummyRender' not in str(env):
                env = DummyRender(env)
...
class PolicyRollouter:
    ...
    def __init__(self, env, save_dir,
                 ...
                 rollout_len_seconds, show_from_end_seconds, no_render_demonstrations):
        self.env = env
        ...
        for _ in range(16):
            self.ctx.Process(target=RolloutWorker, args=(cloudpickle.dumps(make_policy_fn),
                                                         ...
                                                         self.rollout_queue,
                                                         no_render_demonstrations)).start()
