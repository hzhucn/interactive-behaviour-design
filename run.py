...
def eval_loop(env_fn_pkl, policy_fn_pkl,
    ...
    policy_name = policy_name_queue.get(block=True)

    obs, done = None, True
    while True:
        if done:
            try:
                policy_name = policy_name_queue.get(block=False, timeout=0.1)
            except queue.Empty:
                pass
            print("Running eval episodes with policy", policy_name)
            last_ckpt_name = find_latest_checkpoint(ckpt_dir, 'policy-{}-'.format(policy_name))
            policy.load_checkpoint(last_ckpt_name)
            obs = env.reset()
        act = policy.step(obs, deterministic=True)
...
def main():
    ...
    obs_queue = Queue()
    make_basic_env_fn, env = make_env(env_id=args.env, num_env=args.n_envs, seed=args.seed,
                                      experience_dir=experience_dir, episode_obs_queue=obs_queue,
                                      segments_queue=segments_queue, render_segments=args.render_segments,
                                      render_every_nth_episode=args.render_every_nth_episode)
    dummy_env = make_basic_env_fn()
    env.spec = dummy_env.spec
    reset_state_cache.start_saver_receiver()

    global_variables.env_creation_lock = threading.Lock()

    demonstrations_env = make_basic_env_fn()
    ...
    eval_policy_name_queue = multiprocessing.Queue(maxsize=1)
    Process(target=eval_loop, args=(cloudpickle.dumps(make_basic_env_fn),
                                    cloudpickle.dumps(make_policy),
    ...
    policies = PolicyCollection(make_policy, log_dir, demonstration_rollouts, args.seed, eval_policy_name_queue)
