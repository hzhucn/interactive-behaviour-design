...
def net_cnn(s, reuse, training, batchnorm, dropout, regularizer):
    x = s / 255.0
    # Page 15: (Atari)
...
def net_mlp(s, reuse, training, regularizer):
    x = s
    x = tf.layers.dense(x, 64, lambda feat: tf.nn.leaky_relu(feat, alpha=0.01),
                        name='d1', reuse=reuse, kernel_regularizer=regularizer)
    x = tf.layers.dense(x, 64, lambda feat: tf.nn.leaky_relu(feat, alpha=0.01),
                        name='d2', reuse=reuse, kernel_regularizer=regularizer)
    x = tf.layers.dense(x, 1, activation=None,
                        name='d3', reuse=reuse, kernel_regularizer=regularizer)
    x = x[:, 0]
    return x
